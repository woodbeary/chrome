  ...
  try {
    ...
    const requestBody = {
      contents: [{
        parts: [
          { 
            text: createPrompt(context) + '\nIMPORTANT: Respond with ONLY the reply text. No explanations or metadata.' 
          }
        ]
      }],
      generationConfig: {
    ...
    // Handle the new Gemini 1.5 Flash response structure
    ...
    // If we can't find the text in the expected structure, log and throw
    console.error('Unexpected response structure:', data);
    throw new Error(`Unexpected API response structure. Response: ${JSON.stringify(data)}`);
  } catch (error) {
